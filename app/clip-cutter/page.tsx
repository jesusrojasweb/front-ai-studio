...
import { AlertCircle, ChevronRight, HelpCircle, Info, Trash2, X, Check, Film } from "lucide-react"
          ...
          if (progress >= 100) {
            ...
            setTimeout(() => {
              ...
              if (allComplete) {
                setShowToast({
                  type: "success",
                  message: `${Object.keys(fileStatuses).length} videos ready · 100% analyzed`,
                  action: () => {},
                })
              }
            }, 2000)
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-[70%] p-6 overflow-y-auto">
          ...
          <div
            className={`border-2 border-dashed rounded-lg p-10 flex flex-col items-center justify-center cursor-pointer transition-all
              ${dragOver ? "border-green-400 bg-green-400/10 shadow-lg" : "border-gray-700 bg-[#1A1A1A]"}`}
          ...
          >
            <Film size={48} className={`mb-4 ${dragOver ? "text-green-400" : "text-gray-500"}`} />
            <p className="text-lg mb-2">
              Drop MP4 / MOV / WEBM here or <span className="text-green-400">Browse</span>
            </p>
            <p className="text-sm text-gray-400">
              Supported formats: MP4, MOV, WEBM
              <span className="ml-2 inline-flex items-center cursor-help" title="Maximum file size: 2GB">
                <HelpCircle size={14} />
              </span>
            </p>
            <input
                  ...
                  return (
                    <li
                      ...
                      className={`flex items-center p-3 rounded-lg ${
                        fileStatus.status === "error" ? "bg-red-900/20" : "bg-[#1A1A1A]"
                      }`}
                      role="status"
                    >
                      {/* Thumbnail */}
                      <div className="w-16 h-9 bg-gray-800 rounded overflow-hidden mr-3 flex-shrink-0">
                        {fileStatus.thumbnail && (
                          <img
                            ...
                            className={fileStatus.status === "error" ? "blur-sm" : ""}
                          />
                        )}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-1 w-full bg-gray-700 rounded-full h-1.5">
                          <div
                            className={`h-1.5 rounded-full ${
                              fileStatus.status === "error" ? "bg-red-500" : "bg-green-500"
                            }`}
                        ...
                        <div className="mt-1 flex items-center text-sm">
                          {fileStatus.status === "uploading" && (
                            <span className="text-gray-400">Uploading... {fileStatus.progress.toFixed(0)}%</span>
                          )}
                          ...
                          {fileStatus.status === "complete" && (
                            <div className="flex items-center gap-2">
                              ...
                              {fileStatus.badges?.map((badge) => (
                                ...
                                >
                                  {badge === "OK" && <Check size={10} className="mr-1" />}
                                  {badge === "SD" && <AlertCircle size={10} className="mr-1" />}
                                  {badge === "no-audio" && <AlertCircle size={10} className="mr-1" />}
                                  {badge === "OK" ? "OK" : badge === "SD" ? "SD" : "No audio"}
                                </span>
                              ))}
                            </div>
                          )}
        ...
        <div className="w-[30%] border-l border-gray-800 p-6 overflow-y-auto">
          ...
          <div className="bg-[#1A1A1A] rounded-lg p-4 mb-6">
            ...
            <p className="text-sm text-gray-400 mb-3">Learn how to capture footage that performs well on Fanvue.</p>
            ...
            <div className="mt-4 flex items-center">
              <label className="flex items-center cursor-pointer">
                <div className="relative">
                  ...
                  />
                  <div className={`w-10 h-5 rounded-full ${skipWarnings ? "bg-green-500" : "bg-gray-600"}`}></div>
                  <div
                    className={`absolute left-0.5 top-0.5 bg-white w-4 h-4 rounded-full transition-transform ${skipWarnings ? "transform translate-x-5" : ""}`}
                  ></div>
                </div>
      ...
      {showToast && (
        <div
          className={`fixed bottom-6 right-6 p-4 rounded-lg shadow-lg flex items-center gap-3 max-w-md animate-fade-in ${
            showToast.type === "success" ? "bg-green-900/80 text-white" : "bg-red-900/80 text-white"
          }`}
        >
